...
%\item A basic understanding of well-mixed discrete stochastic simulations and models based on ODEs \cite{dan,sundials}. 
...
\warning{The conversion process operates correctly only if the model to be converted is entirely based on \textcolor{blue}{mass action kinetics}. If the model to be converted is \textcolor{blue}{NOT} entirely based on mass action kinetics, the conversion tool only scales the concentrations to population (integer) values, without modifying the rates (parameters).}
...
For stochastic simulations the data are saved in two subfolders of your data folder named trajectories and stats under the results folder.\note[SH]{I didn't understand this. Is the data saved in subfolders of the folder named `data', or somewhere else?} For deterministic simulations, the data are saved in
